package cleantool.su.piccompress.adapter;
...
public class AlbumsAdapter extends BaseAdapter {
    ...
    public AlbumsAdapter(Context context) {
        ...
        ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(context).threadPriority(Thread.NORM_PRIORITY - 2).denyCacheImageMultipleSizesInMemory()
                .discCacheFileNameGenerator(new Md5FileNameGenerator()).tasksProcessingOrder(QueueProcessingType.LIFO).memoryCacheExtraOptions(96, 120).build();
        ...
        options = new DisplayImageOptions.Builder().showStubImage(R.drawable.album_default_loading_pic) // 设置图片下载期间显示的图片
                ...
                .bitmapConfig(Config.RGB_565).imageScaleType(ImageScaleType.IN_SAMPLE_INT).build(); // 创建配置过的DisplayImageOption对象
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            holder = new Holder();
            convertView = layoutInflater.inflate(R.layout.pic_ablums_adapter_item, parent, false);
        ...
        holder.count.setText(arrayList.get(position).getCount() + mContext.getString(R.string.piccompress_count));
        holder.name.setText(arrayList.get(position).getBucketName());

        imageLoader.displayImage("file://" + arrayList.get(position).getImageList().get(0).getImagePath(), holder.image, options);
        return convertView;
    }
